...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['look_message'])) {
        $_SESSION['timestamp'] = $_POST['look_message'];
        $sql_message = "delete from tbl_messages where time_stamp = :timestamp";
        if ($stmt = $conn->prepare($sql_message)) {
            $stmt->bindParam(":timestamp", $_SESSION['timestamp'], PDO::PARAM_STR);
            if ($stmt->execute()) {
                // message uploaded
            }
        }
...
<div class="wrapper">
    ...
    <p> Click a message to delete it. </p>
                ...
                if ($stmt->execute()){
                    ...
                    while ($row = $stmt->fetch()) {
                        if($row['user_id'] == $user_id){
                            ...
                            echo "<button class='container' type='submit' name='look_message' value='".$row['time_stamp']."' >";
